...
Route::get('/test','UserApiController@test1');
Route::group(['middleware' => ['auth:api']], function () {
	// estimated
	Route::get('/estimated/fare' , 'UserApiController@estimated_fare');
	Route::post('/checkDrivingZone' , 'UserApiController@checkDrivingZone');
	// user profile
	...
	Route::get('/getFirstName/{id}' , 'UserApiController@getFirstName');
	Route::get('/getLastName/{id}' , 'UserApiController@getLastName');
	Route::get('/getEmail/{id}' , 'UserApiController@getEmail');
	Route::get('/getMobile/{id}' , 'UserApiController@getMobile');
	Route::get('/getCountryCode/{id}' , 'UserApiController@getCountryCode');
...
//	Route::post('/send/request' , 	'UserApiController@send_request');
	Route::post('/send/requests' , 	'UserApiController@send_request_user');
	...
	// recurrent
	Route::post('/request/recurrent/update', 'UserApiController@update_recurrent');

	Route::get('/get_advertisements', 'Resource\AdvertisementResource@get_user_advertisement');
});
...
Route::post( '/checkPoiPriceLogic', 'UserApiController@checkPoiPriceLogic')->name('checkPoiPriceLogic');

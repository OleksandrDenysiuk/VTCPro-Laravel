...
var ChauffeurBookingForm = function (object, option) {
    ...
    this.init = function () {
        ...
        $self.e('.chbs-location-add').on('click', function (e) {
            ...
            if (field_length > 1) {
                swal(
                    'Error!',
                    'You can insert only 1 WayPoints!',
                    'error'
                );
            } else {
        ...
        $self.e('.chbs-main-content-step-2').on('click', '.chbs-vehicle-list .chbs-button.chbs-button-style-2:not(.chbs-button-on-request)', function (e) {
            ...
            if (promo_percentage != 0) {
                ...
                $('.promocode_price').text(parseFloat(promo_price).toFixed(2) + " " + currency);
    ...
    this.calculatePriceBaseLocationDistanceCustomOther = function () {
        ...
        $.ajax({
            url: '/admin/calculatePriceBaseLocationDistanceCustomOther',
            data: {
    ...
    this.calculatePriceBasePOICustomOther = function () {
        ...
        $.ajax({
            url: '/admin/calculatePriceBasePOICustomOther',
            data: {
                total_kilometer: total_distance,
                total_hours: total_hours,
                total_minutes: total_minutes,
            ...
            success: function (result) {
                let currency = $('input[name="currency"]').val();
                let total_fare = result['data']['estimated_fare'];
                let surge_val = result['data']['surge_val'];
    ...
    this.checkStep1Validation = function () {
        if ($('input[name="chbs_service_type_id"]').val() == 1) {
            if ($('input[name="chbs_pickup_location_service_type_1"]').val() === '') {
                ...
                swal(
                    'Error!',
                    'The Drop location field is required.',
                    'error'
                );
                return false;
            }
            ...
            if (switch_val === true) {
                if ($('input[name="chbs_pickup_date_service_type_2"]').val() === '') {
                    swal(
                        'Error!',
                        'The Return date field is required.',
                        'error'
                    );
                    return false;
                }
                if ($('#return_time_picker').val() === '') {
                    swal(
                        'Error!',
                        'The Return time field is required.',
                        'error'
                    );
                    return false;
                }
                ...
                if (parseFloat(startDateTime) >= parseFloat(returnDateTime)) {
                    swal(
                        'Error!',
                        'Please insert correct return date.',
                        'error'
                    );
                    return false;
                }
    ...
    this.checkStep3Validation = function () {
        if ($('input[name="chbs_client_billing_detail_enable"]').val() == 0) {
            // console.log($('textarea[name="chbs_comments"]').val());
            /*if ($('textarea[name="chbs_comments"]').val() === "") {
                toastr.error("Comment field is required.", 'Error', {
    ...
    this.setSummaryBillingInfo = function () {
        if ($('input[name="chbs_client_billing_detail_enable"]').val() == 0) {
            ...
            if ($('input[name="chbs_client_user_pro_enable"]').val() == 1) {
                $(".company_name_passenger").removeClass('display-hidden');
                $(".address_passenger").removeClass('display-hidden');
                $("#company_name_passenger").text($('input[name="chbs_client_billing_detail_company_name_passenger"]').val());
                $("#address_passenger").text($('input[name="chbs_client_billing_detail_address_passenger"]').val());
            }
            ...
            $("#country_code_passenger").text($('input[name="chbs_client_billing_detail_country_code_passenger"]').val());
            $("#chbs_comment").text($('textarea[name="chbs_comments"]').val());
        }
    };
    /**********************************************************************/
            ...
            success: function (result) {
                ...
                $.each(result, function (index, item) {
                    ...
                    dataHtml +=
                        ...
                        '                <span>\n' +
                        '                    <span id="chbs-vehicle-price-' + item.id + '">' + parseInt(item.fixed).toFixed(2) + '<sup>&euro;</sup></span>\n' +
                        '                </span>\n' +
                        '            </div>\n' +
                        ...
                        '            <div class="chbs-vehicle-content-price">\n' +
                        '                <span>\n' +
                        '                    <span><sup>&euro;</sup>' + parseInt(item.fixed) + '</span>\n' +
                        '                </span>\n' +
                        '            </div>\n' +
                        '            <div class="chbs-vehicle-content-meta">\n' +
                        '                <div class="chbs-vehicle-content-meta-info">\n' +
    ...
    this.getServicePOIDisatnceInfo = function (start_time) {
        ...
        $.ajax({
            url: '/admin/getServicePOIDisatnceInfo',
            data: {
                passenger: passenger,
                suitcase: suitcase,
                vehicle_type: vehicle_type,
    ...
    this.checkUseWallet = function () {
        ...
        var total_fare = parseFloat(document.getElementsByClassName('total_fare')[0].lastChild.textContent.slice(0, -1));
        var promo_percentage = $('input[name="promo_percentage"]').val();
        var promo_max_amount = $('#promo_max_amount').val();
        if (promo_percentage != 0) {
            let promo_price = parseFloat(total_fare * promo_percentage / 100).toFixed(2);
        ...
        if (promo_percentage != '0')
            total_fare = parseFloat(total_fare * promo_percentage / 100).toFixed(2);
        $.ajax({
            url: '/admin/checkUseWallet',
            type: 'GET',

    ...
    public function update_profile(Request $request)
    {
        ...
        try {
            ...
            if ($image = $request->file('picture')) {
                ...
                $allowed_filename =  time() . mt_rand() . '.' . $extension;

//                Storage::disk('local')->put('user/profile', $image);
                $image->move(public_path("/storage/user/profile"), $allowed_filename);
                $user->picture = "user/profile/" . $allowed_filename;
            }
    ...
    {
        if ($serviceList = ServiceType::all()->where('fleet_id', 0)) {
            return $serviceList;
        } else {
    ...
    public function getFirstName($user_id = 0, $company_id = 0)
    {
        if ($user_id == '0') {//for dispatcher.
            if ($company_id == '0') {
                if ($firstName = User::pluck('first_name')) {
                    return $firstName;
                } else {
                    return response()->json(['error' => trans('api.services_not_found')],
                        422);
                ...
                if ($firstName = User::where('company_id', $company_id)->pluck('first_name')) {
                    return $firstName;
                } else {
                    return response()->json(['error' => trans('api.services_not_found')],
                        422);
            ...
            if ($firstName = User::where('id', $user_id)->pluck('first_name')) {
                return $firstName;
            } else {
                return response()->json(['error' => trans('api.services_not_found')],
                    422);
    ...
    public function getLastName($user_id = 0, $company_id = 0)
    {
        if ($user_id == '0') {//for dispatcher.
            if ($company_id == '0') {
                if ($lastName = User::pluck('last_name')) {
                    return $lastName;
                } else {
                    return response()->json(['error' => trans('api.services_not_found')],
                        422);
                ...
                if ($lastName = User::where('company_id', $company_id)->pluck('last_name')) {
                    return $lastName;
                } else {
                    return response()->json(['error' => trans('api.services_not_found')],
                        422);
            ...
            if ($lastName = User::where('id', $user_id)->pluck('last_name')) {
                return $lastName;
            } else {
                return response()->json(['error' => trans('api.services_not_found')],
                    422);
    ...
    public function getEmail($user_id = 0, $company_id = 0)
    {
        if ($user_id == '0') {//for dispatcher.
            if ($company_id == '0') {
                if ($email = User::pluck('email')) {
                    return $email;
                } else {
                    return response()->json(['error' => trans('api.services_not_found')],
                        422);
                ...
                if ($email = User::where('company_id', $company_id)->pluck('email')) {
                    return $email;
                } else {
                    return response()->json(['error' => trans('api.services_not_found')],
                        422);
    ...
    public function getMobile($user_id = 0, $company_id = 0)
    {
        if ($user_id == '0') {//for dispatcher.
            if ($company_id == '0') {
                if ($mobile = User::pluck('mobile')) {
                    return $mobile;
                } else {
                    return response()->json(['error' => trans('api.services_not_found')],
                        422);
                ...
                if ($mobile = User::where('company_id', $company_id)->pluck('mobile')) {
                    return $mobile;
                } else {
                    return response()->json(['error' => trans('api.services_not_found')],
                        422);
    ...
    public function getCountryCode($user_id = 0, $company_id = 0)
    {
        if ($user_id == '0') {//for dispatcher.
            if ($company_id == '0') {
                if ($country_code = User::pluck('country_code')) {
                    return $country_code;
                } else {
                    return response()->json(['error' => trans('api.services_not_found')],
                        422);
                ...
                if ($country_code = User::where('company_id', $company_id)->pluck('country_code')) {
                    return $country_code;
                } else {
                    return response()->json(['error' => trans('api.services_not_found')],
                        422);
            ...
            if ($country_code = User::where('id', $user_id)->pluck('country_code')) {
                return $country_code;
            } else {
                return response()->json(['error' => trans('api.services_not_found')],
                    422);
    ...
    public function getCompanyName($user_id = 0, $company_id = 0)
    {
        if ($user_id == '0') {//for dispatcher.
            if ($company_id == '0') {
                ...
                if ($company_name = User::where('company_id', $company_id)->pluck('company_name')) {
                    return $company_name;
                } else {
                    return response()->json(['error' => trans('api.services_not_found')],
                        422);
            ...
            if ($company_name = User::where('id', $user_id)->pluck('company_name')) {
                return $company_name;
            } else {
                return response()->json(['error' => trans('api.services_not_found')],
                    422);
    ...
    public function getCompanyAddr($user_id = 0, $company_id = 0)
    {
        if ($user_id == '0') {//for dispatcher.
            if ($company_id == '0') {
                if ($company_addr = User::pluck('company_address')) {
                    return $company_addr;
                } else {
                    return response()->json(['error' => trans('api.services_not_found')],
                        422);
                ...
                if ($company_addr = User::where('company_id', $company_id)->pluck('company_address')) {
                    return $company_addr;
                } else {
                    return response()->json(['error' => trans('api.services_not_found')],
                        422);
            ...
            if ($company_addr = User::where('id', $user_id)->pluck('company_address')) {
                return $company_addr;
            } else {
                return response()->json(['error' => trans('api.services_not_found')],
                    422);
    ...
    public function getAllFieldUser(Request $request)
    {
        ...
        if ($option === 'first_name') {
            ...
            $result = User::where('company_name', $company_name)->first();
        } elseif ($option === 'company_addr') {
            $company_addr = $request->input('company_addr');
            $result = User::where('company_address', $company_addr)->first();
        }
    ...
    public function send_request_dispatcher(Request $request)
    {
        ...
        $travelling_type = $request->chbs_service_type_id;   //1: ride, 2: schedule
        $passenger_mail = $request->input('passenger_email_address');
        $passenger_id = User::where('email', $passenger_mail)->value('id');
        $total_price = $request->input('total_price');
        $schedule_return_date = $request->input('schedule_return_date');
        $schedule_return_time = $request->input('schedule_return_time');
        $calculateState = $request->input('calculateState'); // poi or distance
...
//		}
        ...
        if ($travelling_type === '2') //schedule
        {
            ...
            if ($schedule_return_date != null && $schedule_return_time != null) {
                $beforeschedule_time =
                    (new Carbon("$request->schedule_date $request->schedule_time"))->subHour(1);
                $afterschedule_time =
                    (new Carbon("$request->schedule_date $request->schedule_time"))->addHour(1);
        ...
        try {
            ...
            if ($count == 0) {
                ...
                $user->password = bcrypt($request->input('passenger_lastname'));
                $user->email = $request->input('passenger_email_address');
                $user->mobile = $request->input('passenger_phone');
                $user->country_code = $request->input('passenger_country_code');
                if ($request->input('is_user_pro_status') == '1') {
                    $user->company_name = $request->input('passenger_company_name');
                    $user->company_address = $request->input('passenger_address');
...
//			$role = \Illuminate\Support\Facades\Auth::guard()->user()->getRoleNames()->toArray();
            $dispatcher_name = Admin::where('id', Auth::user()->id)->value('name');
            ...
            $UserRequest->d_longitude =
                $request->d_longitude ? $request->d_longitude
                    : $request->s_longitude;
            $UserRequest->fleet_id = User::where('id', $passenger_id)->value('fleet_id');
            ...
            if ($passenger_data->wallet_balance > 0 || $passenger_data->user_type == 'COMPANY') {
                $UserRequest->use_wallet = $request->use_wallet ?: 0;
                if ($passenger_data->user_type == 'COMPANY' && $passenger_data->allow_negative == 1 && $UserRequest->use_wallet == 1) {
            ...
            if ($traveller_type == 'PASSENGER') {
                $UserRequest->passenger_name = $request->input('passenger_name');
                $UserRequest->passenger_phone = $request->input('passenger_phone');
            }
            ...
            if ($schedule_return_date != null && $schedule_return_time != null) {
                ...
                $UserRequest1->fleet_id = User::where('id', $passenger_id)->value('fleet_id');
                if ($request->s_latitude == null && $request->s_longitude == null) {
                ...
                $UserRequest1->unit = config('constants.distance', 'Kms');
                //already saved passenger data

                if ($passenger_data->wallet_balance > 0 || $passenger_data->user_type == 'COMPANY') {
                    $UserRequest1->use_wallet = $request->use_wallet ?: 0;
                    if ($passenger_data->user_type == 'COMPANY' && $passenger_data->allow_negative == 1 && $UserRequest1->use_wallet == 1) {
                ...
                $traveller_type = $request->input('traveller_type', 'TRAVELLER');
                if ($traveller_type == 'PASSENGER') {
                    $UserRequest1->passenger_name = $request->input('passenger_name');
                    $UserRequest1->passenger_phone = $request->input('passenger_phone');
                }
...
//			}
    ...
    public function send_request(Request $request)
    {
        ...
        if (count($Providers) == 0 && !($request->has('schedule_date') && $request->has('schedule_time'))) {
            if ($request->ajax()) {
                // Push Notification to User
        ...
        try {
            ...
            if (Auth::user()->wallet_balance > 0 || Auth::user()->user_type == 'COMPANY') {
                $UserRequest->use_wallet = $request->use_wallet ?: 0;
                if (Auth::user()->user_type == 'COMPANY' && Auth::user()->allow_negative == 1 && $UserRequest->use_wallet == 1) {
                    $UserRequest->payment_mode = 'WALLET';
                }
            }
            ...
            if ($UserRequest->status != 'SCHEDULED') {
                if ((config('constants.manual_request', 0) == 0) && (config('constants.broadcast_request', 0) == 0)) {
                    //Log::info('New Request id : '. $UserRequest->id .' Assigned to provider : '. $UserRequest->current_provider_id);
                    if (count($Providers) > 0) (new SendPushNotification)->IncomingRequest($Providers[0]->id);
                }
            }
            ...
            if ($request->has('schedule_date') && $request->has('schedule_time')) {
                if ($request->has('recurrent') && count($request['recurrent']) > 0) {
                    ...
                    $UserRequestRecurrent->schedule_at = $UserRequest->schedule_at;
                    // $UserRequestRecurrent->repeated = json_encode($request['recurrent']);
                    $UserRequestRecurrent->repeated = json_encode(array_map('intval', $request['recurrent']));
                    $UserRequestRecurrent->save();

                    // $UserRequest->user_req_recurrent_id = $UserRequestRecurrent->id;
                    UserRequests::where('id', $UserRequest->id)->update(['user_req_recurrent_id' => $UserRequestRecurrent->id]);
                }
            }
...
//		$this->checkPendingRequest($passenger_id);
        ...
        if ($travelling_type === '2') //schedule
        {
            $beforeschedule_time = (new Carbon("$request->schedule_return_date $request->schedule_return_time"))->subHour(1);
            $afterschedule_time = (new Carbon("$request->schedule_return_date $request->schedule_return_time"))->addHour(1);
        ...
        try {
            ...
            if ($passenger_data->wallet_balance > '0' || $passenger_data->user_type == 'COMPANY') {
                ...
                if ($passenger_data->allow_negative == '0' && $UserRequest->use_wallet == '1' && $passenger_data->wallet_balance > $total_price) {
                    $UserRequest->payment_mode = 'WALLET';
                } else if ($passenger_data->allow_negative == '1' && $UserRequest->use_wallet == '1' && abs($passenger_data->wallet_balance + $passenger_data->wallet_limit) > $total_price) {
                    $UserRequest->payment_mode = 'WALLET';
                }
            }
            ...
            if ($schedule_return_date != null && $schedule_return_time != null) {
                $UserRequest->total_price = $total_price / 2;
                $UserRequest->promo_price = $promo_price == 0 ? 0 : $promo_price / 2;
                $UserRequest->distance = $request->distance / 2;
            } else {
                $UserRequest->total_price = $total_price;
            ...
            if ($schedule_return_date != null && $schedule_return_time != null) {
                ...
                if ($passenger_data->wallet_balance > 0 || $passenger_data->user_type == 'COMPANY') {
                    $UserRequest1->use_wallet = $request->use_wallet ? $request->use_wallet : 0;
                    if ($passenger_data->allow_negative == 0 && $UserRequest1->use_wallet == 1 && $passenger_data->wallet_balance > $total_price) {
                        $UserRequest1->payment_mode = 'WALLET';
                    } else if ($passenger_data->allow_negative == 1 && $UserRequest1->use_wallet == 1 && ($passenger_data->wallet_balance + $passenger_data->wallet_limit) > $total_price) {
                        $UserRequest1->payment_mode = 'WALLET';
                    }
                }

                $UserRequest1->total_price = $total_price / 2;
                $UserRequest1->promo_price = $promo_price == 0 ? 0 : $promo_price / 2;
                ...
                $UserRequest1->created_by = Auth::user()->first_name . " " . Auth::user()->last_name;
                $UserRequest1->created_id = Auth::user()->id;
                $UserRequest1->calculate_state = $calculateState;
                $UserRequest1->surge_price = $surge_price;
                $UserRequest1->save();
...
//			}
            ...
            // update payment mode
            User::where('id', $passenger_id)
                ->update(['payment_mode' => $request->payment_mode]);

            if ($request->has('card_id')) {

                Card::where('user_id', $passenger_id)
                    ->update(['is_default' => 0]);
                Card::where('card_id', $request->card_id)
                    ->update(['is_default' => 1]);
            }
                ...
                foreach ($UserRequests as $key => $value) {
                    ...
                    if ($value->way_points !== null) {
                        ...
                        $UserRequests[$key]['static_map'] =
							...
							&format=png
							&visual_refresh=true
							&markers=size:mid%7Ccolor:0xff0000%7Clabel:1%7C" . $value->s_address
                            . "&markers=size:mid%7Ccolor:0xff38eb%7Clabel:2%7C" . UserRequests::getWayPointAddress($value->way_points)
                            . "&markers=size:mid%7Ccolor:0x3bff23%7Clabel:3%7C" . $value->d_address
...
//		try{
        ...
        foreach ($point_interest as $key => $val) {
            ...
            if ($start_point_is_polygoon && $dest_point_is_polygoon) {
                return response()->json(['status' => true, 'price' => (string)$val->price, 'service_type_id' => $val->service_type_id, 'service_id' => $val->id]);
            }
            $i++;
            if (count($point_interest) == $i) {
        ...
        try {
            ...
            if ($vehicle_id === '0') {
                ...
                foreach ($data as $key => $val) {
                    ...
                    $data[$key]['fixed'] = (string)DispatcherController::getSurgePriceBasedonDistance($val->id, $response, $start_time);
...
//			$surgePriceByPOIServiceID = PointInterest::where('id',$poi_service_id)->value('ignore_surge_price');  // 1 or 0
            ...
            if ($vehicle_id === '0') {
                ...
                foreach ($data as $key => $val) {
                    if (in_array($val->id, $Poi_service_type_id)) {
//						if($surgePriceByPOIServiceID == '0')
                        $data[$key]['fixed'] = $poiPrice;
                        $data[$key]['description'] = '';
                        $data[$key]['sel'] = $sel;
                        //calculate peak hour
...
//						$data[ $key ]['fixed'] = DispatcherController::getSurgePriceBasedonDistance($val->id, $response,$start_time);
                        $data[$key]['fixed'] = (string)$response['data']['estimated_fare'];
                        $data[$key]['description'] = '';
                        $data[$key]['sel'] = $sel;
                    }
                }
...
//						}
    ...
    public function calculatePriceBasePOICustomOther(Request $request)
    {
        try {
            ...
            $surgePriceByPOIServiceID = PointInterest::where('id', $poi_service_id)->value('ignore_surge_price');  // 1 : applied or 0 : not applied


            if (in_array($service_type, $Poi_service_type_id)) {
//				if($surgePriceByPOIServiceID == '0')
                $return_data['estimated_fare'] = $response->applyNumberFormat(floatval($poiPrice));
...
//				}
    ...
    public function checkUseWallet(Request $request)
    {
        ...
        $usable_wallet = ($user_data->wallet_balance > 0 && $user_data->allow_negative == '0' && abs($passenger_data->wallet_balance + $passenger_data->wallet_limit) > $total_price)
            || ($user_data->user_type == 'COMPANY' && $user_data->allow_negative == 1 && abs($passenger_data->wallet_balance + $passenger_data->wallet_limit) > $total_price);

        if ($usable_wallet) {
            return response()->json(["status" => true, "wallet_amount" => $user_data->wallet_balance]);
        } else {
            return response()->json(["status" => false]);
        }
    }
    ...
    public function checkPromoCodeUsage(Request $request)
    {
        ...
        $promoData = Promocode::where('expiration', '>', date('Y-m-d H:i'))->where('promo_code', $coupon_code)->get();
        if (count($promoData) == 0) {
            return response()->json(['status' => false, 'error' => 'Invalid Promo Code']);
        } else { //already used?
            $promo_id = $promoData[0]->id;
            $usedCount = PromocodeUsage::where('status', 'USED')->where('promocode_id', $promo_id)->where('user_id', $passenger_id)->count();
            if ($usedCount > 0)  //if already used
            {
                return response()->json(['status' => false, 'error' => 'Promo Code was already used.']);
            } else {// if didn't already used.
                if (UserRequests::where('status', '<>', 'CANCELLED')->where(['user_id' => $passenger_id, 'promocode_id' => $promo_id])->count() > 0) {
                    return response()->json(['status' => false, 'error' => 'Promo Code was already used.']);
                }
                return response()->json(['status' => true, 'promo_id' => $promo_id, 'message' => 'Promo Code was applied.', 'percentage' => $promoData[0]->percentage, 'max_amount' => $promoData[0]->max_amount]);
            }
        }
    ...
    public function checkDrivingZone(Request $request)
    {
        ...
        try {
            $country_list = explode(",", DrivingZone::where(['status' => 'country', 'active' => 1])->value('country_list'));

            if (in_array($start_country, $country_list)) {
                return response()->json(['status' => false, 'error' => 'We are sorry but this ' . $start_country . ' does not exist in our driving zone']);
            }

            $start_city_radius = DrivingZone::select(DB::Raw("(6371 * acos( cos( radians('$start_lat') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$start_lng') ) + sin( radians('$lat') ) * sin( radians(latitude) ) ) ) AS distance"))
                ->where('STATUS', 'location')->where('active', '1')->where('radius', '<>', 0)
            ...
            if ($start_city_radius > 0) {
                return response()->json(['status' => false, 'error' => 'We are sorry but this  ' . $start_area . '  does not exist in our driving zone']);
            }
            ...
            if ($request->has('way_location')) {
                ...
                foreach ($way_location as $key => $val) {
                    ...
                    if (in_array($way_country, $country_list)) {
                        return response()->json(['status' => false, 'error' => 'We are sorry but this ' . $way_country . ' does not exist in our driving zone']);
                    }

                    $way_city_radius = DrivingZone::select(DB::Raw("(6371 * acos( cos( radians('$way_lat') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$way_lng') ) + sin( radians('$way_lat') ) * sin( radians(latitude) ) ) ) AS distance"))
                        ->where('STATUS', 'location')->where('active', '1')->where('radius', '<>', 0)
                    ...
                    if ($way_city_radius > 0) {
                        return response()->json(['status' => false, 'error' => 'We are sorry but this  ' . $way_area . '  does not exist in our driving zone']);
                    }
                }
            }

            $dest_location = json_decode($request->input('destination_location'));
            ...
            if (in_array($dest_country, $country_list)) {
                return response()->json(['status' => false, 'error' => 'We are sorry but this ' . $dest_country . ' does not exist in our driving zone']);
            }

            $way_city_radius = DrivingZone::select(DB::Raw("(6371 * acos( cos( radians('$dest_lat') ) * cos( radians(latitude) ) * cos( radians(longitude) - radians('$dest_lng') ) + sin( radians('$dest_lat') ) * sin( radians(latitude) ) ) ) AS distance"))
                ->where('STATUS', 'location')->where('active', '1')->where('radius', '<>', 0)
                ...
                ->count();
            if ($way_city_radius > 0) {
                return response()->json(['status' => false, 'error' => 'We are sorry but this  ' . $dest_area . '  does not exist in our driving zone']);
            }
        } catch (Exception $ex) {
    ...
    {
        $result = $this->request_status_check();
    }
}

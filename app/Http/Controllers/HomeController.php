...
//		{
			...
			if($user_type == "COMPANY" ){
				$first_name   = $userAPI->getFirstName(0,$user->id);
				$last_name    = $userAPI->getLastName(0,$user->id);
				$email        = $userAPI->getEmail(0,$user->id);
				$mobile       = $userAPI->getMobile(0,$user->id);
				$country_code = $userAPI->getCountryCode(0,$user->id);
			}elseif ($user_type == "NORMAL"){
				$first_name   = $userAPI->getFirstName($user->id);
				$last_name    = $userAPI->getLastName($user->id);
				$email        = $userAPI->getEmail($user->id);
				$mobile       = $userAPI->getMobile($user->id);
				$country_code = $userAPI->getCountryCode($user->id);
			}
...
//		}
	...
	public function test(Request $request)
	{
		...
		return $api->estimated_fare($request->all());

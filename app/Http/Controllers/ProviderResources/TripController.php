...
//				(new PaymentController)->payments($id);
		...
		try {
			...
			if ($UserRequest->calculate_state == 'poi'){
				if($UserRequest->surge_price == '1')
					$Tax = ($UserRequest->total_price + $total_waiting_amount + $toll_price) * ($tax_percentage / 100);
				else
					$Tax = ($UserRequest->total_price + $peakamount + $total_waiting_amount + $toll_price) * ($tax_percentage / 100);
			}
			else
				$Tax = ($Fixed + $Distance_fare + $Minute_fare + $peakamount + $total_waiting_amount + $toll_price) * ($tax_percentage / 100);
			...
			if ($UserRequest->calculate_state == 'poi'){
				$Commision = ($UserRequest->total_price + $Tax + $toll_price + $surge) * ($commission_percentage / 100); //11.1 by song commissoin logic  (total + tax )* commision percentage
			}else{
				$Commision = ($Fixed + $Distance_fare + $Minute_fare + $Tax + $toll_price + $surge) * ($commission_percentage / 100); //11.1 by song commissoin logic  (total + tax )* commision percentage
			}
...
//			$ProviderPay = $ProviderPay - ($peak_comm_amount + $waiting_comm_amount ) + $peakamount + $total_waiting_amount  + $toll_price; // by Song 209-11.1 fixed  subtract  peak_comm_amount + waiting_comm_amount
	...
	*/
		...
		if ($UserRequest->paid == 1)
		{
			...
			if ($paymentsRequest->is_partial == 1) {
				...
				if ($paymentsRequest->payment_mode == "CASH") {
					$credit_amount = $paymentsRequest->wallet + $paymentsRequest->tips;   //38.81
				} else {
			...
			{
				$admin_commision = $paymentsRequest->commision; //4.31

				if (!empty($paymentsRequest->fleet_id))    //fleet id of provider
				{
			...
			if ($credit_amount > 0) {
				...
				if ($provider->wallet_balance > 0) {
					$admin_amount = $credit_amount - ($admin_commision + $paymentsRequest->tax);   //$credit_amount = $paymentsRequest->total + $paymentsRequest->tips; (card, wallet)
				} else {
	...
	public function userCreditDebit($amount, $UserRequest, $type = 1)
	{
		...
		$ipdata = array();
		$ipdata['transaction_id'] = $transaction_id;    //request_id
		$ipdata['transaction_alias'] = $transaction_alias; //booking_id
		$ipdata['transaction_desc'] = $msg;                //msg like trip
		$ipdata['transaction_type'] = $transaction_type; // 3
		$ipdata['type'] = $ttype;                        // 'D'
		$ipdata['amount'] = $amount;                     //wallet amount
		$this->createAdminWallet($ipdata);

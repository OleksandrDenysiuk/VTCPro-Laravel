...
//        $this->middleware('permission:service-types-delete', ['only' => ['destroy']]);
    ...
    {
        $services = ServiceType::where('fleet_id',Auth::guard('fleet')->id())->get();
        if($request->ajax()) {
            return $services;
        } else {
    ...
    public function create()
    {
        ...
        $Peakhour =  FleetServicePeakHour::get();
        return view('fleet.service.create', compact('Peakhour'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
	        ...
	        $service->fleet_id = Auth::guard('fleet')->id();
            ...
            if($request->peak_price){
                ...
                foreach ($request->peak_price as $key => $value) {
                    ...
                    if($request->peak_price[$key]>0){
                        $service_map = FleetServicePeakHour::create(['service_type_id'=>$service->id,'peak_hours_id'=>$key,'min_price'=>$request->peak_price[$key]]);
                    }
    ...
    public function edit($id)
    {
        ...
        try {
            ...
            $Peakhour=FleetPeakHour::with(['servicetimes' => function ($query) use ($id) {
            ...
            if($request->peak_price){
                ...
                foreach ($request->peak_price as $key => $value) {
                    ...
                    if($value['status']==1){
                        //update price
                         if($value['id']){
                            $service_map = FleetServicePeakHour::where('service_type_id',$id)->where('peak_hours_id',$key)->update(['min_price'=>$value['id'] ]);
                         }
                         else{
                            //delete peakhours
                            FleetServicePeakHour::where('service_type_id',$id)->where('peak_hours_id',$key)->delete();
                         }
                    }
                    else{
                        if($value['id']){
                            //insert price
                            $service_map = FleetServicePeakHour::create(['service_type_id'=>$id,'peak_hours_id'=>$key,'min_price'=>$value['id']]);
                        }
                    }
                }

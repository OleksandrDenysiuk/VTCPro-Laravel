...
//        $this->middleware('permission:user-delete', ['only' => ['destroy']]);
    ...
    public function index(Request $request)
    {
      /*  if (!empty($request->download) && $request->download == 'all') {
        ...
        if ($request->ajax()) {
            ...
            $users = User::where(['user_type'=>'FLEET_NORMAL','fleet_id'=>Auth::user('fleet')->id])
            ...
            $count = User::where(['user_type'=>'FLEET_NORMAL','fleet_id'=>Auth::user('fleet')->id])
            ->where(function ($query) use ($search_value) {
                $query->orWhere('id', 'like', '%' . $search_value . '%')
                    ->orWhere('first_name', 'like', '%' . $search_value . '%')
                    ->orWhere('last_name', 'like', '%' . $search_value . '%')
            ...
            $total = User::where(['user_type'=>'FLEET_NORMAL','fleet_id'=>Auth::user('fleet')->id])
            ->orderBy(empty($order_name) ? 'id' : $order_name, $dir)
            ->count();
    ...
    public function show($id)
    {
        try {
            ...
            return view('fleet.users.user-details', compact('user'));
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            return redirect()->route('fleet.user.index')->with('flash_success', trans('admin.user_msgs.user_update'));
        } catch (ModelNotFoundException $e) {
            return back()->with('flash_error', trans('admin.user_msgs.user_not_found'));
        }
    }
...
//            $pagination = (new Helper)->formatPagination($requests);
            ...
            return view('fleet.request.index', compact('requests',  'trips','dates','fleet'));
        } catch (Exception $e) {

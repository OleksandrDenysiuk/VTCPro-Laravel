...
class PaymentController extends Controller {
	...
	public function payment( Request $request )
	{
		...
		$this->validate( $request,
			[
				'request_id' => 'required|exists:user_request_payments,request_id|exists:user_requests,id,paid,0,user_id,'
				                . Auth::user()->id,
			] );
		...
		$paymentMode = $request->has( 'payment_mode' )
			? strtoupper( $request->payment_mode ) : $UserRequest->payment_mode;

		$tip_amount = 0;

		$random =
			config( 'constants.booking_prefix' ) . mt_rand( 100000, 999999 );

		if ( $paymentMode != 'CASH' )
		{

			$RequestPayment =
				UserRequestPayment::where( 'request_id', $request->request_id )
					->first();
			...
			if ( $totalAmount == 0 )
			{
				...
				$RequestPayment->payment_mode = $UserRequest->payment_mode;    //12.20 song
				$RequestPayment->provider_pay =
					$RequestPayment->provider_pay + $tip_amount;
				$RequestPayment->save();
				...
				switch ( $paymentMode ) {
					...
					case 'CARD':
						...
						return $gateway->process( [
							...
							"card"          => $Card->card_id,
							"description"   => "Payment Charge for "
							                   . Auth::user()->email,
					...
					case 'PAYUMONEY':
						...
						if ( $request->ajax() ) {
							...
							$paramList['surl']             =
								url( 'api/user/payu/success' );
							$paramList['curl']             =
								url( 'api/user/payu/failure' );
							$paramList['service_provider'] = 'payumoney';
							$paramList['merchant_id']      =
								config( 'constants.payumoney_merchant_id' );
							$paramList['payu_salt']        =
							...
							foreach ( $hashVarsSeq as $hash_var ) {
								$hash_string .= isset( $paramList[ $hash_var ] )
									? $paramList[ $hash_var ] : '';
								$hash_string .= '|';
							}
					...
					case 'PAYPAL-ADAPTIVE':
						...
						if ( $provider->paypal_email != null ) {
							...
							$primary_email     =
								config( 'constants.paypal_email', '' );
							$secondary_email[] = [
								'secondary_email' => $provider->paypal_email,
					...
					case 'PAYTM':
						...
						if ( $request->ajax() ) {
							...
							$callback_url =
								( config( 'constants.paytm_environment' )
								  == 'local' )
							...
							$paramList["MID"]              =
								config( 'constants.paytm_merchant_id' );
							$paramList["ORDER_ID"]         = $random;
							$paramList["CUST_ID"]          =
								(String) Auth::user()->id;
							$paramList["INDUSTRY_TYPE_ID"] =
								config( 'constants.paytm_industry_type' );
							...
							$paramList["CHECKSUMHASH"] =
								PaytmLibrary::getChecksumFromArray( $paramList,
									config( 'constants.paytm_merchant_key' ) );
		...
		switch ( strtoupper( $request->payment_mode ) ) {
			...
			case 'CARD':
				...
				if ( $user_type == 'provider' ) {
					...
					ProviderCard::where( 'user_id', Auth::user()->id )
						->update( [ 'is_default' => 0 ] );
					ProviderCard::where( 'card_id', $request->card_id )
						->update( [ 'is_default' => 1 ] );
				} else {
			...
			case 'PAYTM':
				...
				if ( $request->ajax() ) {
					...
					$callback_url =
						( config( 'constants.paytm_environment' ) == 'local' )
							? 'https://securegw-stage.paytm.in/theia/paytmCallback?ORDER_ID='
							  . $random
							: 'https://securegw.paytm.in/theia/paytmCallback?ORDER_ID='
							  . $random;
			...
			case 'CHEQUE':
				try {
					...
					return back()->with( 'flash_error',
						'Something went wrong. Please check payment method and money' );
				}
				break;
			default:
				return back()->with( 'flash_error',
	...
	public function response( Request $request ) {
		...
		if ( $log->is_wallet == 1 ) {
			...
			if ( $log->user_type == "user" ) {
				...
				$wallet = ( new TripController )->userCreditDebit( $log->amount,
					$user->id,
					1 );
				( new SendPushNotification )->WalletMoney( $user->id,
					currency( $log->amount ) );
			} elseif ( $log->user_type == "provider" ) {
				$user   = \App\Provider::find( $log->user_id );
				$wallet =
					( new TripController )->providerCreditDebit( $log->amount,
						$user->id,
						1 );
				( new SendPushNotification )->ProviderWalletMoney( $user->id,
					currency( $log->amount ) );
			}
			...
			if ( $request->ajax() ) {
				return response()->json( [
					'success'        => currency( $log->amount ) . " "
					                    . trans( 'api.added_to_your_wallet' ),
					'message'        => currency( $log->amount ) . " "
					                    . trans( 'api.added_to_your_wallet' ),
					'wallet_balance' => $wallet_balance,
				] );
			} else {
				if ( $log->user_type == "provider" ) {
					return redirect( '/provider/wallet_transation' )->with( 'flash_success',
						currency( $log->amount )
						. trans( 'admin.payment_msgs.amount_added' ) );
				} else {
					return redirect( 'wallet' )->with( 'flash_success',
						currency( $log->amount )
						. trans( 'admin.payment_msgs.amount_added' ) );
				}
			}
		...
		$RequestPayment               =
			UserRequestPayment::where( 'request_id', $UserRequest->id )
				->first();
	...
	public function failure( Request $request ) {
		...
		if ( $log->is_wallet == 1 ) {
			...
			if ( $request->ajax() ) {
				...
				if ( $request->type == "provider" ) {
					return redirect( '/provider/wallet_transation' )->with( 'flash_error',
						'Transaction Failed' );
				} else {
					return redirect( 'wallet' )->with( 'flash_error',
						'Transaction Failed' );
				}
			}
		}
		...
		if ( $request->ajax() ) {
			...
			if ( $request->type == "provider" ) {
				return redirect( '/' )->with( 'flash_success',
					'Transaction Failed' );
			} else {
				return redirect( 'dashboard' )->with( 'flash_success',
					'Transaction Failed' );
	...
	public function paytm_response( Request $request ) {
		...
		$log           =
			PaymentLog::where( 'transaction_code', $request->ORDERID )->first();
		...
		if ( $request->STATUS == "TXN_SUCCESS" ) {
			return redirect( $provider_url . '/payment/response?order='
			                 . $request->ORDERID . '&pay=' . $request->TXNID );
		} else {
			return redirect( $provider_url . '/payment/failure?order='
			                 . $request->ORDERID );
		}
	}
	...
	public function payu_response( Request $request ) {
		$log           =
			PaymentLog::where( 'transaction_code', $request['txnid'] )->first();
	...
	public function payu_error( Request $request ) {
		...
		$log           =
			PaymentLog::where( 'transaction_code', $request )->first();
	...
	public function payments( $request_id )
	{
		$UserRequest = UserRequests::find( $request_id );

		$paymentMode = $UserRequest->payment_mode;
...
//			}
			...
			if ( $totalAmount == 0 )
			{
				...
				//for create the transaction
				( new TripController )->callTransaction( $request_id );
				...
				switch ( $paymentMode ) {
					...
					case 'CARD':
						...
						$Card = Card::where( 'user_id', $UserRequest->user_id )
							->where( 'is_default', 1 )->first();

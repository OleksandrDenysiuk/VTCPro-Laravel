...
//		( $role[0] === 'DISPATCHER' ) {
			...
			$first_name   = $userAPI->getFirstName();
			$last_name    = $userAPI->getLastName();
			$email        = $userAPI->getEmail();
			$mobile       = $userAPI->getMobile();
			$country_code = $userAPI->getCountryCode();
			$company_name = $userAPI->getCompanyName();
			$company_addr = $userAPI->getCompanyAddr();
...
//		}
	...
	public static function getSurgePriceBasedonDistance($service_id,$response,$start_time)
	{
		$start_time_check =
	...
	public static function calculatePriceBaseLocationDistanceCustom( $total_kilometer,
		$total_hours,
		$total_minutes,
		$service_type
	) {

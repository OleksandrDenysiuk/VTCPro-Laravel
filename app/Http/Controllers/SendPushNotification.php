...
class SendPushNotification extends Controller
{
     ...
     * @return void
     */
    public function FindingDriver($request)
    {
        ...
        return $this->sendPushToUser($request->user_id, trans('user.ride.finding_driver'));
    }

    /**
     * AcceptedScheduledRide 
     *
     * @return void
     */
    public function AcceptedScheduledRide($request)
    {
        ...
        return $this->sendPushToUser($request->user_id, trans('user.ride.accepted_scheduled_ride'));
    }
     ...
     * @return void
     */
    public function CancelledScheduledRide($request)
    {
        ...
        return $this->sendPushToUser($request->user_id, trans('user.ride.canceled_scheduled_ride'));
    }
    ...
    public function sendPushToUser($user_id, $push_message)
    {
        ...
        try {
            ...
            if ($user->device_token != "") {
                ...
                $option = array('sound' => 'default');

                if ($user->device_type == 'ios') {

                    return PushNotification::app('IOSUser')
                        ->to($user->device_token)
                        ->send($push_message, $option);
                } elseif ($user->device_type == 'android') {
                    // GCM removed by Google at
                    // return \PushNotification::app('Android')
                    //     ->to($user->device_token)
                    //     ->send($push_message);

                    Helper::sendFCM($user->device_token, $push_message);
                }
            }
        } catch (Exception $e) {
            return $e;
        ...
        try {
            ...
            if ($provider->token != "") {
                ...
                if ($provider->type == 'ios') {
                    ...
                    //     ->to($provider->token)
                    //     ->send($push_message);
                    Helper::sendFCM($provider->token, $push_message);
                }
            }
        } catch (Exception $e) {
            return $e;

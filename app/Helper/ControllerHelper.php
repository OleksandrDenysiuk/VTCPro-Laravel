...
class Helper
{
	...
	public static function sendFCM($token = '', $msg)
	{
		...
		$ret = $pushManager->push();
		// dd($push->getResponses());
	...
	public static function welcomeEmailToNewUser($user_type = '', $user = null)
	{
		...
		try {
			...
			Mail::send('emails.welcome_register', ['data' => $data], function ($mail) use ($data, $subject) {
				...
				$mail->to($data['email'], $username)->subject($subject);
			});
			// echo $user['email'];
			return true;
		} catch ( Throwable $th) {
	...
	public static function addServiceType()
	{
		...
		$fleetIds = Fleet::select('id')->get();//get fleet ID

		for($i = 0; $i<count($fleetIds);$i++)
		{
			$data =[]; 	$data1=[];
			$id = $fleetIds[$i]->id;
			$fleetServiceTypeName = ServiceType::where('fleet_id',$id)->pluck('name');
			if(count($fleetServiceTypeName) === 0)
			{
				for($j = 0; $j<count($serviceTypeName);$j++)
				{
					$data[] = [
						'name' => $serviceTypeName[$j]->name,
						'provider_name' => $serviceTypeName[$j]->provider_name,
						'image' => $serviceTypeName[$j]->image,
						'marker' => $serviceTypeName[$j]->marker,
						'capacity' => $serviceTypeName[$j]->capacity,
						'luggage_capacity' => $serviceTypeName[$j]->luggage_capacity,
						'fixed' => $serviceTypeName[$j]->fixed,
						'price' => $serviceTypeName[$j]->price,
						'minute' => $serviceTypeName[$j]->minute,
						'hour' => $serviceTypeName[$j]->hour,
						'distance' => $serviceTypeName[$j]->distance,
						'calculator' => $serviceTypeName[$j]->calculator,
						'description' => $serviceTypeName[$j]->description,
						'waiting_free_mins' => $serviceTypeName[$j]->waiting_free_mins,
						'waiting_min_charge' => $serviceTypeName[$j]->waiting_min_charge,
						'fleet_id' => $id,
						'status' => $serviceTypeName[$j]->status,
			...
			{
				for($k=0; $k<count($serviceTypeName); $k++)
				{
					$test = $serviceTypeName[$k]->name;
					$var1 = $fleetServiceTypeName->toArray();
					if(!in_array($test, $var1))
					{
						$data1[] = [
							'name' => $serviceTypeName[$k]->name,
							'provider_name' => $serviceTypeName[$k]->provider_name,
							'image' => $serviceTypeName[$k]->image,
							'marker' => $serviceTypeName[$k]->marker,
							'capacity' => $serviceTypeName[$k]->capacity,
							'luggage_capacity' => $serviceTypeName[$k]->luggage_capacity,
							'fixed' => $serviceTypeName[$k]->fixed,
							'price' => $serviceTypeName[$k]->price,
							'minute' => $serviceTypeName[$k]->minute,
							'hour' => $serviceTypeName[$k]->hour,
							'distance' => $serviceTypeName[$k]->distance,
							'calculator' => $serviceTypeName[$k]->calculator,
							'description' => $serviceTypeName[$k]->description,
							'waiting_free_mins' => $serviceTypeName[$k]->waiting_free_mins,
							'waiting_min_charge' => $serviceTypeName[$k]->waiting_min_charge,
							'fleet_id' => $id,
							'status' => $serviceTypeName[$k]->status,
							'created_at' => Carbon::now()->format('Y-m-d H:i:s'),
							'updated_at' => Carbon::now()->format('Y-m-d H:i:s')
						];
						DB::table('service_types')->insert($data1);
					}
				}
			}
		}

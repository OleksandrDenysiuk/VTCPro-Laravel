...
{{--        <div class="msg-date">@php $dt = new DateTime(); echo $dt->format("d/m/Y - H:i");@endphp</div>--}}
        <div class="msg-icons">
            ...
            @endphp
            <div class="msg-icon-price" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="@lang('admin.fleets.Total_Earnings')">{{number_format($revenue, 2, '.', '').config('constants.currency')}}</div>
        </div>
    </div>
    <div class="mobile-mils">Total: {{number_format($revenue, 2, '.', '').config('constants.currency')}}</div>
    <div class="col-lg-3 col-md-4 msg-rt">
        <div class="msr-wrapc">
            <div class="msg-welcome"><span>@lang('admin.dashboard.Wlc')</span><br>{{Auth::user()->name}}</div>
            <div class="msg-img">

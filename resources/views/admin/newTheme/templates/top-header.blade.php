...
{{--        <div class="msg-date">@php $dt = new DateTime(); echo $dt->format("d/m/Y - H:i");@endphp</div>--}}
        <div class="msg-icons">
            ...
            <div class="msg-icon-price" data-toggle="tooltip" data-placement="bottom" title=""
                 data-original-title="@lang('admin.fleets.Total_Earnings')">{{currency(App\UserRequestPayment::sum('cash') + App\UserRequestPayment::sum('card') + App\UserRequestPayment::sum('wallet'))}}</div>
        </div>
    </div>

    <div class="mobile-mils">
        Total: {{currency(App\UserRequestPayment::sum('cash') + App\UserRequestPayment::sum('card') + App\UserRequestPayment::sum('wallet'))}}</div>

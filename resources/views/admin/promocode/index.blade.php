                ...
                <table class="table table-striped table-bordered dataTable" id="table-2">
                    ...
                    @foreach($promocodes as $index => $promo)
                        <tr>
                            ...
                            <td>
                                {{appDateTime($promo->expiration)}}
                            </td>
                            <td>
                                @if(date("Y-m-d H:i") <= $promo->expiration)
                                    <span class="tag tag-success">@lang('admin.custom.valid')
</span>
                                @else
                                    <span class="tag tag-danger">@lang('admin.custom.expiration')
</span>
                                @endif
                            </td>
                            <td>
                            ...
                            <td>
                                <form action="{{ route('admin.promocode.destroy', $promo->id) }}" method="POST">
                                    ...
                                    @can('promocodes-edit')
                                    <a href="{{ route('admin.promocode.edit', $promo->id) }}" class="btn btn-info"><i class="fa fa-pencil"></i> @lang('admin.custom.edit')
</a>
                                    @endcan
                                    @can('promocodes-delete')
                                    <button class="btn btn-danger" onclick="return confirm('Are you sure?')"><i class="fa fa-trash"></i> @lang('admin.custom.delete')</button>
                                    @endcan
                                    @endif
                                </form>
                            </td>

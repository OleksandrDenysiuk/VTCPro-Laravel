...
{{--            @can('service-types-create')--}}
            <a href="{{ route('admin.pointinterest.create') }}" style="margin-left: 1em;" class="btn btn-primary pull-right"><i class="fa fa-plus"></i>@lang('admin.point.Add_Point_Interest')</a>
{{--            @endcan--}}
            <table class="table table-striped table-bordered dataTable" id="table-2">
                ...
                @foreach($point_interest as $index => $point)
                    <tr>
                        ...
                        <td>
                            <form action="{{ route('admin.pointinterest.destroy', $point->id) }}" method="POST">
                                {{ csrf_field() }}
                                {{ method_field('DELETE') }}
                                @if( Setting::get('demo_mode', 0) == 0)
                                <a href="{{ route('admin.pointinterest.show', $point->id) }}" class="btn btn-success">
                                    <i class="fa fa-book"></i> @lang('admin.poi.show')
                                </a>
                                @can('service-types-edit')
                                <a href="{{ route('admin.pointinterest.edit', $point->id) }}" class="btn btn-info">

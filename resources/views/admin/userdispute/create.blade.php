    ...
    <div class="content-area py-1">
        <div class="container-fluid">
            <div class="box box-block bg-white">
                <a href="{{ route('admin.userdisputes') }}" class="btn btn-default pull-right"><i
                            class="fa fa-angle-left"></i> @lang('admin.back')</a>

                <h5 style="margin-bottom: 2em;">@lang('admin.dispute.add_dispute')</h5>

                <form class="form-horizontal" action="{{route('admin.userdisputestore')}}" method="POST"
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-xs-5">
                            <select class="form-control" name="dispute_type" id="dispute_type">
                                <option value="user">@lang('admin.dispute.dispute_user')</option>
                                <option value="provider">@lang('admin.dispute.dispute_provider')</option>
                            </select>
                        </div>
                    </div>
                        ...
                        <div class="col-xs-5">
                            <div class="input-group">
                                <input class="form-control" type="text" value="{{ old('name') }}" name="name"
                                       id="namesearch" placeholder="@lang('admin.fleet.flesearchs')" required=""
                    ...
                    <div class="form-group row">
                        <label for="lost_item_name"
                               class="col-xs-2 col-form-label">@lang('admin.lostitem.request')</label>
                        <div class="col-xs-5">
                            <table class="table table-striped table-bordered dataTable requestList">
                                ...
                                <tr>
		                            ...
		                            <th>@lang('admin.custom.choose')</th>
                                </tr>
                                </thead>
                                <tbody>
                                <tr>
                                    <td colspan="4">@lang('admin.custom.no_results')</td>
                                </tr>
                                </tbody>
                            </table>
                        </div>
                    ...
                    <div class="form-group row">
                        <label for="lost_item_name"
                               class="col-xs-2 col-form-label">@lang('admin.dispute.dispute_name')</label>
                        <div class="col-xs-5">
                            <select class="form-control" name="dispute_name" id="dispute_name" required="">
                                <option value="">@lang('admin.custom.select')</option>
                            </select>
                            <textarea style="display: none;margin-top:5px;" class="form-control" name="dispute_other"
                                      required id="dispute_other"
                ...
                $.ajax({
                    ...
                    success: function (data, textStatus, jqXHR) {
                        ...
                        if (data.data.length > 0) {
                            ...
                            for (var i in result) {
                                requestList.html(`<tr><td>` + result[i].booking_id + `</td><td>` + result[i].s_address + `</td><td>` + result[i].d_address + `</td><td>
                                        <input name="request_id" value="` + result[i].id + `" type="radio" /><input name="user_id" value="` + result[i].user_id + `" type="hidden" />
                                        <input name="provider_id" value="` + result[i].provider_id + `" type="hidden" /></td></tr>`);
                            }
                        } else {
                            requestList.html(`<tr><td colspan="4">@lang('admin.custom.no_results')</td></tr>`);
                        }
                    }
                });

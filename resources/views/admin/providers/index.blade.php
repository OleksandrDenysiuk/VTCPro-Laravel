                    ...
                    <tr>
                        ...
                        <td>
                            @if($provider->fleet == 0)
                                Admin
                            @else
                                {{ $provider->getFleetName($provider->fleet) }}
                            @endif
                        </td>
                        ...
                        <td>
                            ...
                            @if($provider->active_documents() == $total_documents && $provider->service != null)
                                 <a class="btn btn-success btn-block" href="{{route('admin.provider.document.index', $provider->id )}}">@lang('admin.custom.all_set')</a>
                            @else
                                <a class="btn btn-danger btn-block label-right" href="{{route('admin.provider.document.index', $provider->id )}}">@lang('admin.custom.attention') <span class="btn-label">{{ $provider->pending_documents() }}</span></a>
                            @endif
                        ...
                        <td>
                            @if($provider->service)
                                @if($provider->service->status == 'active')
                                    <label class="btn btn-block btn-primary">@lang('admin.custom.yes')</label>
                                @else
                                    <label class="btn btn-block btn-warning">@lang('admin.custom.no')</label>
                                @endif
                            @else
                        ...
                        <td>
                            <div class="input-group-btn">
                                ...
                                <a class="btn btn-danger btn-block" href="{{ route('admin.provider.disapprove', $provider->id ) }}">@lang('admin.custom.Disable')</a>
                                @else
                                <a class="btn btn-success btn-block" href="{{ route('admin.provider.approve', $provider->id ) }}">@lang('admin.custom.Enable')</a>
                                @endcan
                                @endif

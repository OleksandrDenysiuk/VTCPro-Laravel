...
{{--        <div class="msg-date">@php $dt = new DateTime(); echo $dt->format("d/m/Y - H:i");@endphp</div>--}}
        <div class="msg-icons">
            <div class="msg-icon-img">
                ...
                $fully_sum = App\UserRequestPayment::whereHas('request', function($query) {
                        ...
                        ->sum('provider_pay');
                $notifications = App\Notifications::where([['notify_type','!=','user'],['status','active']])
            ...
            <div class="msg-icon-price" data-toggle="tooltip" data-placement="bottom" title="" data-original-title="@lang('admin.fleets.Total_Earnings')">{{number_format($fully_sum, 2, '.', '').config('constants.currency')}}</div>
        </div>
    </div>
    <div class="mobile-mils">@lang('admin.fleets.Total_Earnings'): {{number_format($fully_sum, 2, '.', '').config('constants.currency')}}</div>
